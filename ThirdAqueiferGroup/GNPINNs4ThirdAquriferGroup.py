...
潜水含水层GWR + PINNs模型
File: GWR-PINNs-1.py
本版本专为潜水含水层设计，包含以下关键调整：
- 使用潜水含水层PDE方程：∂/∂x(Kh∂h/∂x) + ∂/∂y(Kh∂h/∂y) + Q_over + Q_pump = S∂h/∂t
- 水力传导系数K与含水层厚度h相乘
- 针对潜水特性优化的特征处理和参数设置
...
BASE_DIR = r"E:\研究生\PINN-Dissertation-4\AAAAAAAAAAAAGWR-PINNs18-245\数据集\II承压水"
COORD_FILE = r"E:\研究生\PINN-Dissertation-4\AAAAAAAAAAAAGWR-PINNs18-245\数据集\II承压水\训练数据\xls文件\潮白河第II承压含水层坐标.xls"
EXTRACT_FILE = r"E:\研究生\PINN-Dissertation-4\AAAAAAAAAAAAGWR-PINNs18-245\数据集\II承压水\训练数据\xls文件\潮白河第II承压含水层开采量.xls"
RAIN_FILE = r"E:\研究生\PINN-Dissertation-4\AAAAAAAAAAAAGWR-PINNs18-245\数据集\II承压水\训练数据\xls文件\潮白河第II承压含水层越流补给量.xls"
HEAD_FILE = r"E:\研究生\PINN-Dissertation-4\AAAAAAAAAAAAGWR-PINNs18-245\数据集\II承压水\训练数据\xls文件\潮白河第II承压含水层水位值.xls"
...
def two_stage_gwr_pinn():
    ...
    # 随机分割井位，30%作为测试集
    train_wells, test_wells = train_test_split(all_wells, test_size=0.2, random_state=42)
    ...
    X_range = X_max - X_min + 1e-8  # 避免除以零

    H_min = H_train.min(axis=0)
    H_max = H_train.max(axis=0)
    H_range = H_max - H_min + 1e-8  # 避免除以零
    ...
    # build pde residual for unconfined aquifer
    # 潜水含水层方程: ∂/∂x(Kh∂h/∂x) + ∂/∂y(Kh∂h/∂y) + Q_over(越流补给) + Q_pump(开采量) = S∂h/∂t
    H_t = sn.diff(H_orig, tv)
    H_x = sn.diff(H_orig, x); H_y = sn.diff(H_orig, y)
    # 潜水含水层：添加与H_orig的乘积项
    K_Hx = kvar * H_orig * H_x; K_Hy = kvar * H_orig * H_y
    div_x = sn.diff(K_Hx, x); div_y = sn.diff(K_Hy, y)
    rain_min = X_min[5]; rain_max = X_max[5]; rain_range = X_range[5]
    extract_min = X_min[6]; extract_max = X_max[6]; extract_range = X_range[6]
    # 越流补给量（正值）和开采量（负值）
    ...
    # 检查是否存在已保存的模型权重，如果存在则加载
    weights_file = 'two_stage_gwr_pinn_weights.h5'
    if os.path.exists(weights_file):
        print(f"发现已保存的模型权重文件: {weights_file}")
        print("正在加载权重并准备继续训练...")
        try:
            # 加载之前保存的权重
            model.load_weights(weights_file)
            print("模型权重加载成功，可以继续训练！")
        except Exception as e:
            print(f"加载模型权重时出错: {e}")
            print("这可能是由于模型结构（如神经元数量）发生了变化导致的")
            print(f"将删除旧的权重文件 {weights_file} 并从头开始训练...")
            try:
                os.remove(weights_file)
                print(f"成功删除旧权重文件: {weights_file}")
            except Exception as remove_error:
                print(f"删除旧权重文件时出错: {remove_error}")
                print("将尝试从头开始训练...")
    else:
    ...
    # 训练输入包含所有变量
    train_inputs = [x_tr, y_tr, t_tr, sin_tr, cos_tr, rain_tr, extract_tr, k_tr, s_tr, H0_train_s]
    ...
    try:
        results_dict = evaluate_model_predictions(
            ...
            save_dir='E:\研究生\PINN-Dissertation-4\AAAAAAAAAAAAGWR-PINNs18-245\数据集\II承压水\results'
        )
        
        # Save results to Excel
        output_dir = 'E:\研究生\PINN-Dissertation-4\AAAAAAAAAAAAGWR-PINNs18-245\数据集\II承压水'
...
def residual_correction_gtwr_pinn():
    ...
    print(f"数据集构建完成，不包含空间高阶特征。特征维度: {X.shape[1]}")
    ...
    phys = Functional('nn_phys', [x,y,tv,tsin,tcos,r,e,kvar,svar], 
                    #   ,x_sq,y_sq,xy_prod
                      ...
                      kernel_regularizer=tf.keras.regularizers.l2(1e-1))  # 适度的L2正则化
    
    H_trial_expr = h0_var + tv * phys
    H_orig = H_trial_expr * H_range + H_min
    H_t = sn.diff(H_orig, tv); H_x = sn.diff(H_orig, x); H_y = sn.diff(H_orig, y)
    # 潜水含水层：添加与H_orig的乘积项
    K_Hx = 30 * kvar * H_orig * H_x; K_Hy = 30 * kvar * H_orig * H_y
    # 潜水含水层方程: ∂/∂x(Kh∂h/∂x) + ∂/∂y(Kh∂h/∂y) + Q_over(越流补给) + Q_pump(开采量) = S∂h/∂t
    pde_residual = sn.diff(K_Hx, x) + sn.diff(K_Hy, y) + (r*X_range[5]+X_min[5]) + (e*X_range[6]+X_min[6]) - svar*H_t
    
    # 平衡的自适应损失权重策略 - 增加数据权重以降低数据损失
    data_weight = 2.0  # 增加数据权重初始值，让模型更重视数据拟合
    pde_weight = 10.0  # 适中的初始PDE权重，保持物理约束
    ...
    model = SciModel([x,y,tv,tsin,tcos,r,e,kvar,svar, h0_var], 
                    #  ,x_sq,y_sq,xy_prod,h0_var
    ...
    # 检查是否存在已保存的模型权重，如果存在则加载
    weights_file = 'baseline_pinn_best_weights.h5'
    if os.path.exists(weights_file):
        print(f"发现已保存的模型权重文件: {weights_file}")
        print("正在加载权重并准备继续训练...")
        try:
    ...
    callbacks = [
        # 更合理的早停机制，避免过早停止
        EarlyStopping(monitor='loss', patience=100, restore_best_weights=True, min_delta=5e-7),
        # 添加TensorBoard监控
        tf.keras.callbacks.TensorBoard(log_dir='logs/baseline_pinn')
    ]

    # 优化的动态损失权重调整回调 - 增加数据权重调整功能
    class EnhancedAdaptiveLossWeights(tf.keras.callbacks.Callback):
        ...
        def on_epoch_end(self, epoch, logs=None):
            ...
            if epoch > 0 and epoch % self.update_freq == 0:
                ...
                # 基于训练阶段调整PDE权重
                # 可修改参数：早期阶段的epoch阈值
                early_stage_epochs = 10000  # 可根据需要调整此值
                if epoch < early_stage_epochs:  # 早期阶段延长
                    # 逐渐增加PDE权重，从0.5到2.0
                    progress = min(epoch / early_stage_epochs, 1.0)
                    ...
                    if self.stagnation_count >= 1:
                        ...
                        if len(self.data_loss_history) > 10 and len(self.pde_loss_history) > 10:
                            ...
                            current_pde_weight = self.pde_weight_var
                            current_data_weight = self.data_weight_var
                            
                            # 当PDE损失已经很低时，增加数据权重以降低数据损失
                            pde_loss_threshold = 0.02  # PDE损失阈值，低于此值认为PDE损失已足够低
                            
                            if recent_pde_loss < pde_loss_threshold:
                                # PDE损失已经很低，增加数据权重以降低数据损失
                                new_data_weight = tf.minimum(current_data_weight * 1.2, 5.0)
                                self.data_weight_var.assign(new_data_weight)
                                # 安全地获取Tensor值
                                try:
                                    weight_val = new_data_weight.numpy() if hasattr(new_data_weight, 'numpy') else tf.keras.backend.get_value(new_data_weight)
                                    print(f"  PDE损失已足够低 ({recent_pde_loss:.6f} < {pde_loss_threshold:.6f})，增加数据权重到 {weight_val:.3f}")
                                except Exception as e:
                                    print(f"  PDE损失已足够低 ({recent_pde_loss:.6f} < {pde_loss_threshold:.6f})，增加数据权重")
                            
                            # 同时调整PDE权重
                            if recent_data_loss > recent_pde_loss * 2:
                                # 数据损失太大，增加PDE权重以平衡
                                new_pde_weight = tf.minimum(current_pde_weight * 1.3, 5.0)
                            elif recent_pde_loss > recent_data_loss * 2:
                                # PDE损失太大，减少PDE权重
                                new_pde_weight = tf.maximum(current_pde_weight * 0.7, 0.05)
                            else:
                                # 轻微调整以避免陷入局部最小值
                                # 在图模式下，我们需要使用tf.random而不是np.random
                                adjustment_factor = 0.95 + 0.1 * tf.random.uniform([])
                                new_pde_weight = current_pde_weight * adjustment_factor
    ...
    model.train(
        ...
        adaptive_weights={'method': 'NTK', 'freq': 100},  # 使用sciann支持的NTK方法
        callbacks=callbacks
    )
    ...
    X_train, X_test, H_train, H_test = train_test_split(X_combined, H_combined, test_size=0.2, random_state=42)
    
    # Normalize the test and train data using maximum-minimum normalization
    ...
    H_pred_test_std = model.predict([x_test_norm, y_test_norm, t_test_norm, sin_test, cos_test, rain_test_n, extract_test_n, k_test_n, s_test_n, H0_test_s])[0]
    H_pred_test = H_pred_test_std * H_range + H_min
    
    H_pred_train_std = model.predict([x_train_norm, y_train_norm, t_train_norm, sin_train, cos_train, rain_train_n, extract_train_n, k_train_n, s_train_n, H0_train_s])[0]
    ...
    for well_idx in range(len(x_all)):
        for ti in range(nt):
            ...
            all_predict_inputs.append([x_norm, y_norm, t_norm, sin_norm, cos_norm, r_norm, e_norm, k_norm, s_norm, h0_norm[0]])
            all_predict_indices.append((well_idx, ti))
    
    all_predict_inputs = np.array(all_predict_inputs)
    
    # 一次性预测所有数据点，不再使用高阶空间特征
...
def neural_gwr_pinn_end2end():
    ...
    X_range = X_max - X_min + 1e-8  # 防止除以零
    H_min = H_train.min(axis=0); H_max = H_train.max(axis=0)
    H_range = H_max - H_min + 1e-8  # 防止除以零
    ...
    model_dir = 'neural_gwr_pinn_tf_saved'
    if os.path.exists(model_dir):
        print(f"发现已保存的模型目录: {model_dir}")
        print("正在加载模型并准备继续训练...")
        try:
            # 加载之前保存的模型
            model = tf.keras.models.load_model(model_dir, custom_objects={'NeuralGWR': NeuralGWR})
            print("模型加载成功，可以继续训练！")
        except Exception as e:
            print(f"加载模型时出错: {e}")
            print("将从头开始训练模型...")
    ...
    # Normalize data for prediction (使用最大最小值归一化)
    X_min = X_train.min(axis=0); X_max = X_train.max(axis=0)
    X_range = X_max - X_min + 1e-8  # 防止除以零
    H_min = H_train.min(axis=0); H_max = H_train.max(axis=0)
    H_range = H_max - H_min + 1e-8  # 防止除以零
    ...
    def tf_model_predict(X_input):
        ...
        H_pred_std = model.predict([xy_input, t_input, feat_input, k_input, s_input, h0_input], verbose=0)
...
def evaluate_model_predictions(model, data, X_min, X_max, X_range, H_min, H_max, H_range, X_train, X_test, H_train, H_test, 
    ...
    if isinstance(model, sn.SciModel):
        ...
        H0_train_s = (H0_train - H_mean) / H_std
    ...
    Htrain_pred = Htrain_pred_std * H_std + H_mean
    ...
    for well_idx in range(n_points):
        for ti in range(nt):
            ...
            x_norm = (x_val - X_mean[0]) / X_std[0]
            y_norm = (y_val - X_mean[1]) / X_std[1]
            t_norm = (t_val - X_mean[2]) / X_std[2]
            sin_norm = (sin_t - X_mean[3]) / X_std[3]
            cos_norm = (cos_t - X_mean[4]) / X_std[4]
            r_norm = (r_val - X_mean[5]) / X_std[5]
            e_norm = (e_val - X_mean[6]) / X_std[6]
            k_norm = (k_val - X_mean[7]) / X_std[7]
            s_norm = (s_val - X_mean[8]) / X_std[8]
            h0_norm = (data['head_all'][well_idx, 0] - H_mean) / H_std
    ...
    all_pred = all_pred_std * H_std + H_mean
